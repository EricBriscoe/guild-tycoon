...
import {
  ...
  CLICK_UPGRADES,
  AXES as CFG_AXES,
  PICKAXES as CFG_PICKAXES,
  AUTOMATION as CFG_AUTOMATION,
  AUTOMATION_T2 as CFG_AUTOMATION_T2,
  AUTOMATION_T3_FORGE as CFG_AUTOMATION_T3_FORGE,
  AUTOMATION_T3_WELD as CFG_AUTOMATION_T3_WELD,
  AUTOMATION_T4_WHEEL as CFG_AUTOMATION_T4_WHEEL,
  AUTOMATION_T4_BOILER as CFG_AUTOMATION_T4_BOILER,
  AUTOMATION_T4_COACH as CFG_AUTOMATION_T4_COACH,
  AUTOMATION_T4_LUMBERJACK as CFG_AUTOMATION_T4_LUMBERJACK,
  AUTOMATION_T4_SMITHY as CFG_AUTOMATION_T4_SMITHY,
  AUTOMATION_T4_MECHANIC as CFG_AUTOMATION_T4_MECHANIC
} from './config.js';

// Re-export shared config so UI and calculations reference the same source data
export const AXES: ReadonlyArray<Axe> = CFG_AXES as any;
export const PICKAXES: ReadonlyArray<Pickaxe> = CFG_PICKAXES as any;
export const AUTOMATION: Record<string, AutomationType> = CFG_AUTOMATION as any;
export const AUTOMATION_T2: Record<string, AutomationType> = CFG_AUTOMATION_T2 as any;
export const AUTOMATION_T3_FORGE: Record<string, AutomationType> = CFG_AUTOMATION_T3_FORGE as any;
export const AUTOMATION_T3_WELD: Record<string, AutomationType> = CFG_AUTOMATION_T3_WELD as any;
export const AUTOMATION_T4_WHEEL: Record<string, AutomationType> = CFG_AUTOMATION_T4_WHEEL as any;
export const AUTOMATION_T4_BOILER: Record<string, AutomationType> = CFG_AUTOMATION_T4_BOILER as any;
export const AUTOMATION_T4_COACH: Record<string, AutomationType> = CFG_AUTOMATION_T4_COACH as any;
export const AUTOMATION_T4_LUMBERJACK: Record<string, AutomationType> = CFG_AUTOMATION_T4_LUMBERJACK as any;
export const AUTOMATION_T4_SMITHY: Record<string, AutomationType> = CFG_AUTOMATION_T4_SMITHY as any;
export const AUTOMATION_T4_MECHANIC: Record<string, AutomationType> = CFG_AUTOMATION_T4_MECHANIC as any;
...
// AXES and Tier 1 AUTOMATION are sourced from config.ts

// Tier 2 tools and automation
// PICKAXES and Tier 2 AUTOMATION are sourced from config.ts
...
export function t3ClickUpgradeCost(role: 'forger' | 'welder', level: number): number {
  ...
  const conf = CLICK_UPGRADES.TIER_3;
  if (role === 'forger') return Math.floor(conf.FORGER.baseCostPipes * Math.pow(conf.FORGER.growth, level));
  return Math.floor(conf.WELDER.baseCostBoxes * Math.pow(conf.WELDER.growth, level));
}

// Tier 3 Automation (5 per role)
// Tier 3 automation is sourced from config.ts
...
export function t4ClickUpgradeCost(role: 'lumberjack' | 'smithy' | 'wheelwright' | 'boilermaker' | 'coachbuilder' | 'mechanic', level: number): number {
  ...
  const conf = CLICK_UPGRADES.TIER_4;
  if (role === 'lumberjack') return Math.floor(conf.LUMBERJACK.baseCostWood * Math.pow(conf.LUMBERJACK.growth, level));
  if (role === 'smithy') return Math.floor(conf.SMITHY.baseCostSteel * Math.pow(conf.SMITHY.growth, level));
  if (role === 'wheelwright') return Math.floor(conf.WHEELWRIGHT.baseCostWheels * Math.pow(conf.WHEELWRIGHT.growth, level));
  if (role === 'boilermaker') return Math.floor(conf.BOILERMAKER.baseCostBoilers * Math.pow(conf.BOILERMAKER.growth, level));
  if (role === 'coachbuilder') return Math.floor(conf.COACHBUILDER.baseCostCabins * Math.pow(conf.COACHBUILDER.growth, level));
  return Math.floor(conf.TRAINASSEMBLER.baseCostTrains * Math.pow(conf.TRAINASSEMBLER.growth, level));
}

// Tier 4 Automation (5 per role)
// Tier 4 automation is sourced from config.ts
